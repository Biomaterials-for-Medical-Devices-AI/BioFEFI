...
def test_find_non_numeric_columns():
    ...
    assert result_df_all_numeric == [], "Failed: Should return empty list for all numeric DataFrame"
    assert result_series_numeric is None, "Failed: Should return None for fully numeric Series"
    assert result_series_non_numeric == "NonNumericSeries", "Failed: Should return the series name for non-numeric values"